...
class TestGroupsApi(BaseTest):
    ...
    def group(self, app, client):
        ...
        group = client.groups.create(name)
        yield group
        assert client.groups.delete(group.id), u'The group should be deleted.'
    ...
    def test_edit(self, app, client, group):
        ...
        assert client.groups.edit(group.id, new_name), u'The group name has been edited.'
        ...
        client.groups.edit(group.id, previous_name)
    ...
    def test_add_delete_list_group_users(self, client, group):
        ...
        client.groups.add_user(group.id, user.id)

        user_list = client.groups.list_users(group.id)
        ...
        client.groups.delete_user(group.id, user.id)

        new_user_list = client.groups.list_users(group.id)
